...
;(function($) {
        ...
        Template = $('<div class="rangePicker"> \
        				<div class="wrap"> \
							<div class="preset">\
                                  ...
                                  <footer> \
                                        <button type="button" class="confirm btn btn-sm btn-success">Apply</button> \
                                        <button type="button" class="cancel btn btn-sm btn-default">Cancel</button> \
                                  </footer> \
							</div> \
							<div class="custom"> \
    ...
    RangePicker.prototype = {
        ...
        init: function() {
            ...
            this.picker.add(this.obj)
                .on('mousedown', function(e) {
                    e.stopPropagation()
                });

            this.readonly(this.obj);

            var that = this,
                presetWrap = this.picker.find('.preset > div.content'),
                presetDefault = null,
                years = '',
                months = '',
            ...
            this.calendar.from.append(
                $('<select>')
                    .html(years),  // years.replace(/%%/g,'From - ')
                $('<div>')
                    .addClass('months')
                    .html(months)
            );
        ...
        bind: function() {
            ...
            this.picker
                .on('click', '.preset > .content > button', that.presetClick.bind(that))
        ...
        changePreset: function(val) {
            ...
            if(val) {
                this.presets.removeClass('selected')
                    .filter('[value=' + val + ']')
                    .addClass('selected');
                // set the result
                ...
                if(val == 'custom') {
                    ...
                    var diff_m = (val + '').replace(/^(\d{0,})m/gi, '$1');

                    var d = new Date();
                    d.setMonth(d.getMonth() - 1);
                    var n_y = d.getFullYear();
                    var n_m = d.getMonth() + 1;

                    d.setMonth(n_m - diff_m);
                    var p_y = d.getFullYear();
                    var p_m = d.getMonth() + 1;

                    this.result = [[p_m, p_y], [n_m, n_y]];

                    this.picker.find('.months')
                        .find('.selected')
        ...
        validMonthsInYear: function(reset) {
            ...
            this.yearSelectors.each(function() {
                ...
                if(this.selectedIndex == 0) {
                    monthsElements.slice(0, that.settings.minDate[0] - 1)
                        .prop('disabled', true);
                }

                // for LAST year
                if(this.selectedIndex == (this.length - 1)) {
                    monthsElements.slice(that.settings.maxDate[0])
                        .prop('disabled', true);
                }
        ...
        changeYear: function(e) {
            ...
            if(calendarIdx == 1) {
                this.picker.find('.calendar.from')
                    .find('.selected')
                    .trigger('click.dp');
            }
        ...
        changeMonth: function(calendarIdx, monthIdx) {
            var that = this,
                monthBtn = this.picker.find('.calendar')
                    .eq(calendarIdx)
                    .find('button')
                    .eq(monthIdx);
        ...
        summary: function(calendarIdx) {
            ...
            this.calendar.from.find('strong')
                .html(from);
            this.calendar.to.find('strong')
                .html(to);
        },
        ...
        dateformat: function(date, format) {
            ...
            format = format.replace('yyyy', date[1]);
            format = format.replace('MM', this.leftpad(date[0], 2));
            return format;
        },

        leftpad: function(/**{string}*/ s, /**{int}*/ l, /**[string]*/ symbol) {
            symbol = symbol || '0';
            s = String(s);
        ...
        displayValue: function(format) {
            format = format || '%S - %E';
            format = format.replace('%S', this.dateformat(this.result[0]));
            format = format.replace('%E', this.dateformat(this.result[1]));
            return format;
        },
